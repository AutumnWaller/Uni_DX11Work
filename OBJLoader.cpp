...
bool OBJLoader::FindSimilarVertex(const StaticStructs::StandardVertex& vertex, std::map<StaticStructs::StandardVertex, unsigned short>& vertToIndexMap, unsigned short& index)
{
...
{
	// Mapping from an already-existing StaticStructs::StandardVertex to its corresponding index
	std::map<StaticStructs::StandardVertex, unsigned short> vertToIndexMap;

	std::pair<StaticStructs::StandardVertex, unsigned short> pair;

	int numVertices = inVertices.size();
	
	for(int i = 0; i < numVertices; ++i) //For each vertex
	{
		StaticStructs::StandardVertex vertex = {inVertices[i], inNormals[i],  inTexCoords[i]}; 
...
MeshData OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			StaticStructs::StandardVertex* finalVerts = new StaticStructs::StandardVertex[meshVertices.size()];
			...
			bd.ByteWidth = sizeof(StaticStructs::StandardVertex) * meshVertices.size();
			...
			meshData.VBOffset = 0;
			meshData.VBStride = sizeof(StaticStructs::StandardVertex);
			...
			outbin.write((char*)finalVerts, sizeof(StaticStructs::StandardVertex) * numMeshVertices);
	...
	else
	{
		...
		StaticStructs::StandardVertex* finalVerts = new StaticStructs::StandardVertex[numVertices];
		unsigned short* indices = new unsigned short[numIndices];
		binaryInFile.read((char*)finalVerts, sizeof(StaticStructs::StandardVertex) * numVertices);
		...
		ZeroMemory(&bd, sizeof(bd));
		bd.Usage = D3D11_USAGE_DEFAULT;
		bd.ByteWidth = sizeof(StaticStructs::StandardVertex) * numVertices;
		bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bd.CPUAccessFlags = 0;
